...
class Citation(object):
    ...
    def __repr__(self):
        stats = self.get_fields('title', 'doi', 'journal', 
            'cited_by_posts_count', 'cited_by_tweeters_count', 'cited_by_msm_count', 'cited_by_videos_count', 
            'cited_by_fbwalls_count', 'cited_by_gplus_count',
            'cited_by_rdts_count', 'cited_by_feeds_count','score', 'url')
        ...
        return ('Altmetrics on: "{0}" with doi {1} published in {2}.\n\n' 
                ...
                '{21:11} {22:>5}\n').format(stats[0], stats[1], stats[2],
                            'All Posts', stats[3], 'Tweets', stats[4], 'Mainstream Media', stats[5], 'Videos', stats[6], 
                            'Facebook', stats[7], 'Google+', stats[8],
                            'Reddit', stats[9], 'Blogs', stats[10], 'Score', stats[11], 'url', stats[12])
